...
const ortalamaGelir = (people.reduce((acc, person) => acc + parseInt(person.salary),0) / people.length);
console.log(ortalamaGelir);

// 2) Şu anda 30 yaşından büyük kişiler kimlerdir?
console.log(new Date());
console.log(new Date('02/05/2021'));

let date = new Date();
result = date.toUTCString();
console.log(result);

const yasFiltrele = (item) => (new Date().getFullYear() - new Date(item.DOB).getFullYear()) > 30 ;
const yas = people.filter(yasFiltrele);
console.log(yas);
// veya
const filtreleme = people.filter(item => (new Date().getFullYear() - new Date(item.DOB).getFullYear()) > 30);
...
const sırala = people.sort((personA , personB) => new Date(personB.DOB) - new Date(personA.DOB));
console.log(sırala);
